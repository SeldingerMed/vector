...
| "Gate G binds a signal world W really publishes" | `AuditedEnv` signal surface read first-hand at `world_pin`, path/line verified against the git-fetched tree by `scripts/check_world_signals.py` | Proves the key is assigned where the catalog says, at that pin, in a publication shape. That the assignment actually *reaches* `info`/`extras` is the first-hand read recorded in `safety_evidence`, not something the machine proves — see the row below. Scoped to **(pin, env)**: says nothing about a sibling scene, an unaudited scene, or another revision. Does not validate that the engine's number is physically accurate. |
